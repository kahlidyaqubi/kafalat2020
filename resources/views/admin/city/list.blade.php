...
@section('navigation3','إدارة المدن')
@section('navigation1_link','/admin/home')
@section('navigation3_link','/admin/cities')
            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-bordered table-hover" id="table">
                    ...
                    @foreach($cities->sortByDesc('created_at') as  $city)
                        <tr>
                            <td width="10%"> {{ $i }} </td>
                            <td> {{ ((isset($city->governorate)) && (!is_null($city->governorate))) ? $city->governorate->name : 'غير مدخل' }}</td>
                            <td> {{ $city->name }} </td>
                            <td> {{ $city->name_tr }} </td>
                            <td>
                                <div class="dropdown dropdown-inline">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item Confirm"
                                           href="{{ url('admin/cities/delete/'.$city->id ) }}"><i
                                                    class="fa fa-trash"></i>حذف</a>
                                        <a class="dropdown-item" href="{{ url('admin/cities/'.$city->id .'/edit') }}"><i
