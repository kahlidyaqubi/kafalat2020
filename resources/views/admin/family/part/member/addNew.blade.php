<tr>
    ...
    <td>
        <button class="btn btn-info" the_id="{{$member->person->id}}" onclick="editRow(this)"> <i class="fa fa-edit" aria-hidden="true"></i></button>
        <form action="{{ url('admin/families/removeMember/'.$member->id) }}" method="post">
            @csrf
            <button  type="submit" class="btn btn-danger d-none"> حق</button>
        </form>
         <form action="{{ url('admin/families/removeMember/'.$member->id) }}" method="post">
            @csrf
