...
@section('pageTitle','إدارة أنوع الوثائق')
@section('headerCSS')
...
@section('navigation3','إدارة أنوع الوثائق')
@section('navigation1_link','/admin/home')
@section('navigation3_link','/admin/idTypes')
            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-bordered table-hover" id="table">
                    ...
                    <tr class="text-center">
                        <th class="big-col">#</th>
                        <th class="big-col"> نوع الوثيقة بالعربي</th>
                        <th class="big-col">النوع</th>
                        <th class="big-col">عدد الخانات</th>
                        <th width="15%">العمليات</th>
                    </tr>
                    </thead>
                    <tbody>
                    @php $i = 1 @endphp
                    @foreach($idTypes->sortByDesc('created_at') as $idType)
                        <tr>
                            <td width="10%"> {{ $i }} </td>
                            <td> {{ $idType->name }} </td>
                            <td> {{ $idType->type ==0 ? 'عدد' :'نص' }} </td>
                            <td> {{ $idType->number }} </td>
                            <td>
                                <div class="dropdown dropdown-inline">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item Confirm"
                                           href="{{ url('admin/idTypes/delete/'.$idType->id) }}"><i
                                                    class="fa fa-trash"></i>حذف</a>
                                        <a class="dropdown-item"
                                           href="{{ url('admin/idTypes/'.$idType->id .'/edit') }}"><i
