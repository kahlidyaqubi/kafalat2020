            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-bordered table-hover" id="table">
                    ...
                    <tr class="text-center">
                        <th class="big-col">#</th>
                        <th class="big-col"> وضع الحالة بالعربي</th>
                        <th class="big-col"> وضع الحالة بالتركي</th>
                        <th width="15%">العمليات</th>
                    </tr>
                    </thead>
                    <tbody>
                    @php $i = 1 @endphp
                    @foreach($familyStatuses->sortByDesc('created_at') as $familyStatus)
                        <tr>
                            <td width="10%"> {{ $i }} </td>
                            <td> {{ $familyStatus->name }} </td>
                            <td> {{ !is_null($familyStatus->name_tr) ? $familyStatus->name_tr : '-' }} </td>
                            <td>
                                <div class="dropdown dropdown-inline">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item Confirm"
                                           href="{{ url('admin/familyStatuses/delete/'.$familyStatus->id) }}"><i
                                                    class="fa fa-trash"></i>حذف</a>
                                        <a class="dropdown-item"
                                           href="{{ url('admin/familyStatuses/'.$familyStatus->id .'/edit') }}"><i
