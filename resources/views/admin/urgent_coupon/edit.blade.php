...
@section('pageTitle','تعديل مساعدة طارئة')
@section('headerCSS')
...
@section('navigation3','تعديل مساعدة طارئة')
@section('navigation1_link','/admin/home')
@section('navigation2_link','/admin/urgent_coupons')
@section('navigation3_link','/admin/urgent_coupons/'.$urgent_coupon->id.'/edit')
@section('content')
    <div class="col-lg-12 col-xl-12">
        ...
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                <form method="post" action="/admin/urgent_coupons/{{$urgent_coupon->id}}"
                    ...
                    <div class="row">
                                    ...
                                    <select required class="form-control kt-select2 select2-multi"
                                        ...
                                        @foreach($admin_statuses as $admin_status)
                                            <option value="{{$admin_status->id}}"
                                                    ...
                                                    @else
                                                    @if($admin_status->id == $urgent_coupon->admin_status_id) selected @endif
                        ...
                        <div class="col-lg-3 col-md-6 col-sm-12">
                            <div class="form-group">
                                ...
                                <input class="form-control datepicker-custome" placeholder="yyyy-mm-dd" readonly="readonly" type="text"
                                       name="his_date" required  value="{{old('his_date')??$urgent_coupon->his_date}}"
                                       style="width: 86%">
                            </div>
                        </div>
                        <!-- End col -->
                                    ...
                                    <select required class="form-control kt-select2 select2-multi"
                                        ...
                                        @foreach($coupon_reasons as $coupon_reason)
                                            <option value="{{$coupon_reason->id}}"
                                                    ...
                                                    @else
                                                    @if($coupon_reason->id == $urgent_coupon->coupon_reason_id) selected @endif
                                    ...
                                    <div class="kt-radio-inline">
                                        <label class="kt-radio">
                                            <input type="radio" name="delivery_status"
                                                   @if(!($urgent_coupon->delivery_status)||($urgent_coupon->delivery_status == 0))checked
                                                   @endif
                                                   value="0"> لا
                                            <span></span>
                                        </label>
                                        <label class="kt-radio">
                                            <input type="radio" name="delivery_status"
                                                   @if($urgent_coupon->delivery_status == 1)checked @endif
                                                   value="1"> نعم
                                            <span></span>
                                        </label>
                            ...
                            <div class="form-group">
                                ...
                                <div style="width: 95%;">
                                    <select required class="form-control kt-select2 select2-multi"
                                        ...
                                        <option value="1" oka="{{$urgent_coupon->coupon_type}}"
                                                @if(old('coupon_type'))
                                                {{ 1 == old('coupon_type') ? 'selected':'' }}
                                                @else
                                                @if($urgent_coupon->coupon_type == 1)selected @endif
                                                @endif> نقدية
                                        </option>
                                        <option value="2" oka="{{$urgent_coupon->coupon_type}}"
                                                @if(old('coupon_type'))
                                                {{ 2 == old('coupon_type') ? 'selected':'' }}
                                                @else
                                                @if($urgent_coupon->coupon_type == 2)selected @endif
                                                @endif> عينية
                                        </option>
                                    </select>
                                </div>
                        ...
                        <!-- Start col -->
                        <div class="row type1" @if($urgent_coupon->coupon_type && $urgent_coupon->coupon_type == 1) @else style="display: none" @endif>
                            <div class="col-lg-4  ">
                                <div class="form-group row">
                                    ...
                                    <div style="width: 85%;">
                                        <input type="number" step="0.00001" class="form-control" name="amount"
                                               value="{{old('amount')??$urgent_coupon->amount}}"
                                        ...
                                        <select   class="form-control kt-select2 select2-multi"
                                            ...
                                            @foreach($currencies as $currency)
                                                <option value="{{$currency->id}}"
                                                        @if(old('amount_currency_id'))
                                                        {{ $currency->id == old('amount_currency_id') ? 'selected':'' }}
                                                        @else
                                                        @if($currency->id==$urgent_coupon->amount_currency_id) selected @endif
                                                        @endif>
                        ...
                        <?php  $i = 1?>
                        @if($urgent_coupon->coupon_item_types->first())
                            @foreach($urgent_coupon->coupon_item_types as $coupon_item_type)
                                <div class="row type2">
                                    <!-- Start col -->
                                    <div class="col-lg-2">
                                        <div class="form-group row">
                            ...
                            @endforeach
                        @else
                            <div class="row type2" @if($urgent_coupon->coupon_type && $urgent_coupon->coupon_type == 2) @else style="display: none" @endif>
                                <!-- Start col -->
                                <div class="col-lg-2">
                                    <div class="form-group row">
                                        <label class="col-form-label col-lg-12">اختر التصنيف<span
                    ...
                    <!-- End Row -->
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12">
                                <div class="form-group row">
                                    <label class="col-form-label col-lg-12">نوع التمويل</label>
                                    <div style="width: 95%;">
                                        <div class="kt-radio-inline">
                                            <label class="kt-radio">
                                                <input type="radio" name="funder_type" id="noCheck"
                                                       onclick="javascript:yesnoCheck();"
                                                       @if($urgent_coupon->funder_type == 0 || !($urgent_coupon->funder_type))checked
                                                       @endif
                                                       value="0"> غير ممولة
                                                <span></span>
                                            </label>
                                            <label class="kt-radio">
                                                <input type="radio" name="funder_type" id="yesCheck"
                                                       onclick="javascript:yesnoCheck();"
                                                       @if($urgent_coupon->funder_type == 1)checked @endif
                                                       value="1"> ممولة
                                                <span></span>
                                            </label>
                                        ...
                                        <select class="form-control kt-select2 select2-multi"
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{$country->id}}"
                                                        ...
                                                        @else
                                                        @if($country->id == $urgent_coupon->country_id) selected @endif
                                ...
                                <div class="form-group col">
                                    ...
                                    <div style="width: 95%;">
                                        <select id='selUser2' name="sponsor_id" style='width: 200px;'>
                                            ...
                                            @if($urgent_coupon->sponsor)
                                                <option value="{{$urgent_coupon->sponsor_id}}"
                                                        selected>{{$urgent_coupon->sponsor->name}}</option>
                                            @endif
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group col">
                                    ...
                                    <div style="width: 85%;">
                                        <input type="number" step="0.00001" class="form-control" name="funder_value"
                                               value="{{old('funder_value')??$urgent_coupon->funder_value}}"
                                    ...
                                    <div style="width: 95%;">
                                        <select class="form-control kt-select2 select2-multi"
                                            ...
                                            @foreach($currencies as $currency)
                                                <option value="{{$currency->id}}"
                                                        @if(old('currency_id'))
                                                        {{ $currency->id == old('currency_id') ? 'selected':'' }}
                                                        @else
                                                        @if($currency->id==$urgent_coupon->currency_id) selected @endif
                                                @endif>{{$currency->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                    ...
                    @if($urgent_coupon->urgent_coupon_media->first())
                        @foreach($urgent_coupon->urgent_coupon_media as $media)
                            <!-- Start col -->
                                    ...
                                    <div class="box-img"
                                        ...
                                        <div class="overlay-box">
                                            <div class="option-icons">
                                                ...
                                                <a href="{{ url('admin/urgent_coupons/removeMedia/'.$media->id) }}"
                                                   class="Confirm"><i
...
@section('footerJS')
    ...
    <script>
        var i = parseInt('{{$urgent_coupon->item_types->count()}}');
